...
|------|---------|-------------|
| `--fps` | `0.5` | Mantenuto per backward compatibility |
| `--max-frames` | `30` | Max frame estratti |
...
| `--beam-size` | `5` | Whisper beam search size — aumenta accuratezza, riduce velocità |
...
- `rag_timeline` — each node carries `text_clean`, `keywords`, `embedding_hint`, and `visual_delta`
- `whisper_model` and `vision_model` tracked dynamically in metadata (not hardcoded)
- `language_detected` auto-detected by Whisper (overridable via `--language`)
...
### Scene detection
`process_video.py` uses the ffmpeg `select='gt(scene,0.4)'` filter to detect visual transitions and extract only scene-change frames. If no changes exceed the threshold (common in static or low-motion videos), a single keyframe at `t=0` is used as fallback. Threshold adjustable via `--similarity-threshold`.
